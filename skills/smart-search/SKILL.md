...
- `gemini search-api`
  **首选。** 纯 API 调用（无需浏览器），Google Search grounding，适合全球网页、中英文通用搜索。
  命令：`opencli gemini search-api "<查询>"`
- `grok`
  适合实时讨论、英文互联网舆论、Twitter/X 语境、热点追踪。需要浏览器。
- `doubao`
  适合中文语境、字节抖音生态、生活方式内容、中文热点与泛中文问答。需要浏览器。
- `gemini ask`
  浏览器版 Gemini 对话，需要 Chrome 登录。**仅在 search-api 不可用时使用。**

如果用户没有指定网站，**默认使用 `opencli gemini search-api`**（无需浏览器，最快最稳）。只有当需要特定平台的结果时才切换到其他源。
